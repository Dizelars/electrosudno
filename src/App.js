    ...
    circles: [
      {
        bottom: "27.5%",
        right: "21.4%",
        hoverImgSrc: "/images/kievskyPoint.svg",
      },
      {
        bottom: "69.5%",
        right: "62.4%",
...
// В зависимости от размера устройства в видеотегах должны быть разные видосы
const initVideos = () => {
  const isMobile = window.innerWidth < 481;

  document.querySelectorAll(".bg-video").forEach((el, i) => {
    const source = document.createElement("source");
    source.setAttribute("src", `/${i + 1}${isMobile ? "mob" : "video"}.mp4`);
...
function App() {
  useEffect(() => {
          ...
          infoElem.innerHTML = `
            <div class="card">
              <div class="card_marker">
                <img class="card_marker-img" src="${nextVideo.getAttribute(
                  "data-info-src"
                )}" alt="snow">
              </div>
              <div class="card_descr">
                <p class="card_descr-text">${nextVideo.getAttribute(
                  "data-info-text"
                )}</p>
                <p class="card_descr-title">${nextVideo.getAttribute(
                  "data-info-title"
                )}</p>
              </div>
            </div>
    ...
    const routeImages = document.querySelectorAll(
      "img[data-route]:not(.img-disabled)"
    );
    ...
    const handleAddCircle = ({ bottom, right, hoverImgSrc }) => {
      ...
      const hoverImgRight = isMobile
        ? `${right}`
        : `calc(${right} - ${IMAGE_WIDTH_OFFSET}px`;
    ...
    routeImages.forEach((routeEl) => {
      routeEl.addEventListener("click", (e) => {
        ...
        handleOpenRoute({
          src: e.target.src,
          routeName: routeEl.dataset.route,
        });
      });
    });
